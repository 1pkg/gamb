...
func TestOrd(t *testing.T) {
	table := map[string]struct {
		...
		"ord operator should produce expected result on multiple vars": {
			in: []Var{
				NewVar(10, 20, 30),
				NewVar(1, 2, 3, 5, 10),
				NewVar(2, 3, 4),
			},
			fun: func(v ...interface{}) bool { return v[0].(int)+v[1].(int)-v[2].(int) == 31 },
			out: NewVar(30, 5, 4),
		},
		"ord operator should produce expected result on multiple vars long sequence": {
			in: []Var{
				NewVar(10, 20, 100, 30, 40, 55, 100, 99, 50),
				NewVar(1, 2, 3, 5, 11),
				NewVar(2, 50, 3, 4, 0),
			},
			fun: func(v ...interface{}) bool { return v[0].(int)*v[1].(int)+v[2].(int) == 550 },
		...
		"ord operator should produce expected result on single var": {
			in: []Var{
				NewVar(11, 15, 21, 30),
			},
			fun: func(v ...interface{}) bool { return v[0].(int)%5 == 0 },
		...
		"ord operator should skip empty vars and produce expected result on multiple vars": {
			in: []Var{
				NewVar(100, 200, 300),
				NewVar(),
				NewVar(),
		...
		"ord operator should produce expected result on multiple unequal vars": {
			in: []Var{
				NewVar("1"),
				NewVar("2", "3"),
				NewVar("4", "5", "6"),
			...
			fun: func(v ...interface{}) bool { return v[3].(string)+v[2].(string)+v[1].(string)+v[0].(string) == "A631" },
			out: NewVar("1", "3", "6", "A"),
		},
		"ord operator should produce empty result on if there is no match": {
			in: []Var{
				NewVar(10, 20, 30),
				NewVar(1, 2, 3),
				NewVar(10, 20, 30),
		...
		"ord should operator never panic": {
			in: []Var{
				NewVar("1"),
				NewVar("2", "3"),
				NewVar("4", "5", "6"),
	...
	for tname, tcase := range table {
		t.Run(tname, func(t *testing.T) {
			out := Ord(tcase.fun, tcase.in...)
			if !reflect.DeepEqual(tcase.out, out) {
				t.Fatalf("ord expected result %v but got %v", tcase.out, out)
			}
		})
	}
}
